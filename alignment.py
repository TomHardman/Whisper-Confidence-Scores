...
def align_sequences(source_tokens, target_tokens, sent_edits = False, for_whisper=True):
    # check if sent is OK
...
if __name__ == '__main__':
    wmodel = whisper.load_model('small.en', device='cuda', download_root='/home/mifs/th624/.cache/whisper', dropout=0.0, ilm=False, lora=0)
    wtokenizer = whisper.tokenizer.get_tokenizer(not 'small.en'.endswith('.en'), language='en', task='transcribe', num_languages=wmodel.num_languages)
    hyp_sent = "this is the presentation material methods the transport used from two thousand five to two thousand fifteen and the prediction for two thousand twenty five"
    ref_sent = "this is the presentation material of methods of the transport used from two thousand five to two thousand fifteen and the prediction for two thousand twenty five"
    std = EnglishTextNormalizer()
    hyp_tok = [(i) for i in wtokenizer.encode(hyp_sent)]
    ref_tok = [(i) for i in wtokenizer.encode(ref_sent)]
    token_align(hyp_tok, ref_tok, wtokenizer)
    print('a')